...
func (h *createPoolHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	err = CreatePool(Uuid, request.Currency, request.Language, request.PoolName, request.Amount)
	if err != nil {
		w.WriteHeader(500)
		w.Write([]byte("Something went wrong"))
		return
...
func (h *addPersonHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err := SendEmailJoinedPool(newPerson, &currentPool); err != nil {
...
func SendEmailJoinedPool(p *Person, pool *Pool) error {
	err := godotenv.Load()
	if err != nil {
		return err
	}
...
func (h *performLotteryHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	err = SendEmailLottery(result, &pool)
	if err != nil {
		w.WriteHeader(500)
		w.Write([]byte("Internal server error"))
		return
...
func ComposeEmail(reciever, subject string, body []string) []byte {
	msg := fmt.Sprint("To: %s\r\n", reciever)
...
func SendEmailLottery(lottery map[Person]Person, pool *Pool) error {
	err := godotenv.Load()
	if err != nil {
		return err
	}
	...
	for person, buyTo := range lottery {
		...
		case "sv":
			sub, lines = SwedishLotteryMail(&person, &buyTo, pool)
		case "en":
			sub, lines = EnglishLotteryMail(&person, &buyTo, pool)
		}

		msg := ComposeEmail(person.FirstName, sub, lines)
